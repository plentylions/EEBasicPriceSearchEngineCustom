...
namespace EEBasicPriceSearchEngineCustom\Generator;

use ElasticExport\Helper\ElasticExportStockHelper;
use EEBasicPriceSearchEngineCustom\Helper\PriceHelper;
 ...
 * @package EEBasicPriceSearchEngineCustom\Generator
 */
class BasicPriceSearchEngine extends CSVPluginGenerator
{
    ...
    protected function generatePluginContent($elasticSearch, array $formatSettings = [], array $filter = [])
    {
        ...
        if($elasticSearch instanceof VariationElasticSearchScrollRepositoryContract)
        {
            ...
            do
            {
                $this->getLogger(__METHOD__)->debug('EEBasicPriceSearchEngineCustom::logs.writtenLines', [
                    'Lines written' => $limit,
                ]);
                ...
                $this->getLogger(__METHOD__)->debug('EEBasicPriceSearchEngineCustom::logs.esDuration', [
                    'Elastic Search duration' => microtime(true) - $esStartTime,
                ]);

                if(count($resultList['error']) > 0)
                {
                    $this->getLogger(__METHOD__)->error('EEBasicPriceSearchEngineCustom::logs.occurredElasticSearchErrors', [
                ...
                if(is_array($resultList['documents']) && count($resultList['documents']) > 0)
                {
                    ...
                    foreach($resultList['documents'] as $variation)
                    {
                        ...
                        {
                            $this->getLogger(__METHOD__)->info('EEBasicPriceSearchEngineCustom::logs.variationNotPartOfExportStock', [
                        ...
                        {
                            $this->getLogger(__METHOD__)->error('EEBasicPriceSearchEngineCustom::logs.fillRowError', [
                    ...
                    $this->getLogger(__METHOD__)->debug('EEBasicPriceSearchEngineCustom::logs.buildRowDuration', [
        ...
        $this->getLogger(__METHOD__)->debug('EEBasicPriceSearchEngineCustom::logs.fileGenerationDuration', [
    ...
    private function buildRow($variation, KeyValue $settings)
    {
        ...
        if(!is_null($priceList['variationRetailPrice.price']) && $priceList['variationRetailPrice.price'] > 0)
        {
            ...
            $data = [
                'Category'    => $this->elasticExportHelper->getCategoryBranch($variation['data']['defaultCategories'][0]['id'], $settings, 1),
                ...
                'ImgURL'      => $imageList['normal']['url'],
                'Deeplink1'   => $this->elasticExportHelper->getMutatedUrl($variation, $settings, true, false),
                'Shipping'    => $shipmentAndHandling,
                'Keywords'    => $variation['data']['texts']['keywords'],
                'ArtNumber'   => $variation['data']['item']['id'],
                'Currency'    => $this->elasticExportPriceHelper->getPriceList($variation, $settings, 2, '.')['currency'],
                'PriceUnit'   => number_format((float)$basePriceList['price'], 2, ',', '')
            ];
        ...
        {
            $this->getLogger(__METHOD__)->info('EEBasicPriceSearchEngineCustom::logs.variationNotPartOfExportPrice', [
                'VariationId' => $variation['id']
            ]);
        }
    }
